...
export default function AddJoiningLetter() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success("Joining Letter created successfully ✅", {
        duration: 2000,
        icon: "✅",
      });

      navigate("/dashboards/hrm/joining-letter");
    } catch (err) {
      console.error("Error creating Joining Letter:", err);
      toast.error(err?.response?.data?.message || "Failed to create Joining Letter ❌");
    } finally {
      setSubmitting(false);
    }
  };

  // Permission Gate: PHP code requires permission 249 to Add Joining Letters
  if (!permissions.includes(249)) {
    return (
      <Page title="Add Joining Letter">
        <div className="flex h-60 items-center justify-center rounded-xl border border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-900/20">
          <p className="text-sm font-medium text-red-600 dark:text-red-400">
            Access Denied - Permission 249 required to add joining letters
          </p>
        </div>
      </Page>
    );
  ...
  if (loadingOptions) {
    return (
      <Page title="Add Joining Letter::.Joining Process-Hrm">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">
                Issue a new employment joining letter and configure payroll structure
              </p>
            </div>
          </div>
          {permissions.includes(246) && (
