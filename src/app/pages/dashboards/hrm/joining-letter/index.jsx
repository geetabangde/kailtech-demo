...
export default function ViewJoiningLettersList() {
  const permissions = getStoredPermissions();

  const [joiningLetters, setJoiningLetters] = useState([]);
  const [loading, setLoading] = useState(true);

  // Resilient API fetching for joining letters list
  const fetchJoiningLetters = useCallback(async () => {
    try {
      ...
      if (response.data.status && Array.isArray(response.data.data)) {
        setJoiningLetters(response.data.data);
      } else if (Array.isArray(response.data)) {
        setJoiningLetters(response.data);
      } else {
        setJoiningLetters([]);
      }
    } catch (err) {
  ...
  const table = useReactTable({
    ...
    meta: {
      fetchData: fetchJoiningLetters,
      updateData: (rowIndex, columnId, value) => {
        skipAutoResetPageIndex();
        setJoiningLetters((old) =>
          old.map((row, index) => {
            if (index === rowIndex) {
              return {
                ...old[rowIndex],
  ...
  if (!permissions.includes(246)) {
    return (
      <Page title="View Joining Letters List">
  ...
  if (loading) {
    return (
      <Page title="View Joining Letters List::.Joining Process-Hrm">
