...
export default function ViewJoiningLetter() {
  const { id } = useParams();
  ...
  // Fetch joining letter detail with resilient endpoint fallbacks
  const fetchDetail = useCallback(async () => {
    ...
    if (success && data) {
      setJoiningLetter(data);
    } else {
      toast.error("Failed to load joining letter details ❌");
    }
    setLoading(false);
  }, [id]);
  ...
  const getDesignationName = () => {
    if (!joiningLetter) return "—";
    if (joiningLetter.designation_name) return joiningLetter.designation_name;
    if (joiningLetter.designation && typeof joiningLetter.designation === "object") {
      return joiningLetter.designation.name || "—";
    }
    const matched = designations.find(
      (d) => String(d.id || d.value) === String(joiningLetter.designation)
    );
    return matched ? matched.name || matched.label : joiningLetter.designation || "—";
  };

  // Resolve Branch Name & Location
  const getBranchName = () => {
    if (!joiningLetter) return "—";
    if (joiningLetter.branch_name) return joiningLetter.branch_name;
    if (joiningLetter.branch && typeof joiningLetter.branch === "object") {
      const name = joiningLetter.branch.name || "";
      const loc = joiningLetter.branch.location || "";
      return name && loc ? `${name}/${loc}` : name || "—";
    }
    const matched = branches.find(
      (b) => String(b.id || b.value) === String(joiningLetter.branch)
    );
    if (matched) {
    ...
    return joiningLetter.branch || "—";
  };

  // Gated on Permission 246
  if (!permissions.includes(246)) {
    return (
      <Page title="View Joining Letter">
        <div className="flex h-60 items-center justify-center rounded-xl border border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-900/20">
          <p className="text-sm font-medium text-red-600 dark:text-red-400">
            Access Denied - Permission 246 required to view joining letters
          </p>
        </div>
      </Page>
    );
  ...
  if (loading) {
    return (
      <Page title="View Joining Letter::.Joining Process-Hrm">
    ...
    return (
      <Page title="View Joining Letter::.Joining Process-Hrm">
        <div className="flex h-60 flex-col items-center justify-center gap-4 rounded-xl border border-gray-200 bg-gray-50 dark:border-dark-600 dark:bg-dark-800">
          ...
          <Button onClick={() => navigate("/dashboards/hrm/joining-letter")} variant="outline">
            ← Back to List
          </Button>
        </div>
      </Page>
  ...
  const candidateName = `${joiningLetter.prefix || ""} ${joiningLetter.firstname || ""} ${
    joiningLetter.middlename ? joiningLetter.middlename + " " : ""
  }${joiningLetter.lastname || ""}`.trim();

  const companyName = joiningLetter.companyname || "Kailtech Test & Research Centre";
  const addedByUser = joiningLetter.added_by_name || joiningLetter.added_by || "HR Manager";
  const signatureDate = joiningLetter.added_on ? formatDateTime(joiningLetter.added_on) : formatDate(joiningLetter.offerletterdate);
      ...
      <div className="transition-content p-6 space-y-6">
        ...
        <div className="flex items-center justify-between no-print border-b border-gray-100 dark:border-dark-600 pb-4">
          <div className="flex items-center gap-3">
            <button
              onClick={() => navigate("/dashboards/hrm/joining-letter")}
          ...
          <Card className="page-sheet p-8 md:p-12 bg-white text-gray-800 dark:bg-white dark:text-gray-900 border-none shadow-soft relative overflow-hidden">
            ...
            <div className="relative z-10 space-y-8 h-full flex flex-col justify-between">
              ...
              <div className="flex items-start justify-between border-b border-gray-100 pb-6">
                <div>
                  <img
                    src={joiningLetter.companylogo || "/images/logo.png"}
                    alt="Company Logo"
                ...
                <div className="text-right">
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    Date: {formatDate(joiningLetter.offerletterdate)}
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4 text-sm leading-relaxed">
                ...
                <p className="mt-4">
                  ...
                  would be <span className="font-bold text-gray-950">₹{joiningLetter.gross}</span> and your approximate
                  date of joining would be <span className="font-bold text-gray-950">{formatDate(joiningLetter.joiningdate)}</span>.
                </p>
              </div>
                ...
                <div>
                  ...
                  <div className="space-y-1 mt-3">
                    ...
                    <p>
                      ...
                      <span className="font-bold text-gray-950">{joiningLetter.duration} Months</span>, which may
                ...
                <table className="w-full text-xs border border-gray-300 border-collapse table-fixed">
                  ...
                  <tbody>
                    ...
                    <tr className="font-extrabold bg-gray-50 text-gray-950">
                      ...
                      <td className="border border-gray-300 p-2 text-right font-black">₹{joiningLetter.gross}</td>
                    </tr>
                    {/* Components */}
                    <tr>
                      <td className="border border-gray-300 p-2 text-center">1</td>
                      <td className="border border-gray-300 p-2 text-gray-800">Basic Salary Per Month</td>
                      <td className="border border-gray-300 p-2 text-right">₹{joiningLetter.basic}</td>
                    </tr>
                    <tr>
                      <td className="border border-gray-300 p-2 text-center">2</td>
                      <td className="border border-gray-300 p-2 text-gray-800">H.R.A Per Month</td>
                      <td className="border border-gray-300 p-2 text-right">₹{joiningLetter.hra}</td>
                    </tr>
                    <tr>
                      ...
                      <td className="border border-gray-300 p-2 text-right">₹{joiningLetter.sa}</td>
                    </tr>
                    <tr>
                      <td className="border border-gray-300 p-2 text-center">4</td>
                      <td className="border border-gray-300 p-2 text-gray-800">Bonus</td>
                      <td className="border border-gray-300 p-2 text-right">₹{joiningLetter.bonus}</td>
                    </tr>
                    ...
                    <tr>
                      ...
                      <td className="border border-gray-300 p-2 text-right text-red-600">-₹{joiningLetter.epfemp}</td>
                    </tr>
                    <tr>
                      <td className="border border-gray-300 p-2 text-center">2</td>
                      <td className="border border-gray-300 p-2 text-gray-800">ESIC Employee contribution per Month (0.75% on Gross)</td>
                      <td className="border border-gray-300 p-2 text-right text-red-600">-₹{joiningLetter.esicemp}</td>
                    </tr>
                    <tr>
                      <td className="border border-gray-300 p-2 text-center">3</td>
                      <td className="border border-gray-300 p-2 text-gray-800">Professional Tax (PT)</td>
                      <td className="border border-gray-300 p-2 text-right text-red-600">-₹{joiningLetter.pt}</td>
                    </tr>
                    ...
                    <tr className="font-extrabold bg-emerald-50 text-emerald-950">
                      ...
                      <td className="border border-gray-300 p-2 text-right text-emerald-700 font-black">₹{joiningLetter.netinhand}</td>
                    </tr>
                    ...
                    <tr>
                      ...
                      <td className="border border-gray-300 p-2 text-right">₹{joiningLetter.mobileallowance}</td>
                    </tr>
                    <tr>
                      <td className="border border-gray-300 p-2 text-center">2</td>
                      ...
                      <td className="border border-gray-300 p-2 text-right">₹{joiningLetter.epfemployer}</td>
                    </tr>
                    <tr>
                      <td className="border border-gray-300 p-2 text-center">3</td>
                      ...
                      <td className="border border-gray-300 p-2 text-right">₹{joiningLetter.esicemployer}</td>
                    </tr>
                    ...
                    <tr className="font-bold bg-gray-50 text-gray-900">
                      ...
                      <td className="border border-gray-300 p-2 text-right">₹{joiningLetter.grossctcmonth}</td>
                    </tr>
                    <tr className="font-extrabold bg-blue-950 text-white dark:text-white">
                      ...
                      <td className="border border-blue-950 p-2 text-right text-yellow-400 font-black">₹{joiningLetter.grossctcannual}</td>
                    </tr>
                    ...
                    <tr className="font-bold bg-gray-50 text-gray-900">
                      <td className="border border-gray-300 p-2 text-center">—</td>
                      <td className="border border-gray-300 p-2 uppercase">Net In Hand Salary</td>
                      <td className="border border-gray-300 p-2 text-right">₹{joiningLetter.netinhand}</td>
                    </tr>
                    ...
                    <tr>
                      ...
                      <td className="border border-gray-300 p-2 text-gray-700">
                        Confirmation after <span className="font-bold text-gray-950">{joiningLetter.duration} Months</span> of joining
