...
const confirmMessages = {
  pending: {
    description:
      "Are you sure you want to delete this joining letter? Once deleted, it cannot be restored.",
  },
  success: {
    title: "Joining Letter Deleted",
  },
};
...
export function RowActions({ row, table }) {
  ...
  const handleDeleteRows = useCallback(async () => {
    ...
    try {
      ...
      setDeleteError(true);
      toast.error("Failed to delete joining letter ❌", {
        duration: 2000,
      });
    } finally {
  ...
  return (
    <>
      <div className="flex items-center justify-center gap-2">
        {canView && (
          <Link
            to={`/dashboards/hrm/joining-letter/view/${row.original.id}`}
