  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get("/dashboard/get-master-instrument-due", {
          headers: {
  ...
  const columns = useMemo(() => [
    {
      id: "sr",
      header: "Sr",
      cell: (info) => <span className="text-red-500">{info.row.index + 1}</span>,
    },
    {
      accessorKey: "name",
      header: "Name",
    ...
    <div className="col-span-12 flex flex-col lg:col-span-8 xl:col-span-9">
      <div className="table-toolbar flex items-center justify-between mb-3">
        <h2 className="truncate text-base font-medium tracking-wide text-gray-800 dark:text-dark-100">
          Master Instrument Due In 30 Days
        </h2>
                ...
                {table.getHeaderGroups().map((headerGroup) => (
                  <Tr key={headerGroup.id}>
                    {headerGroup.headers.map((header) => (
                      <Th
                        key={header.id}
                        className="sticky top-0 z-10 bg-gray-200 font-semibold uppercase text-gray-800 dark:bg-dark-800 dark:text-dark-100 first:ltr:rounded-tl-lg last:ltr:rounded-tr-lg first:rtl:rounded-tr-lg last:rtl:rounded-tl-lg"
                      >
