...
export default function EditMom() {
  ...
  // Fetch existing MOM data on mount
  useEffect(() => {
    ...
    const fetchMomDetails = async () => {
      try {
        setSubmitting(true);
        const res = await axios.get(`/master/mom-details/${id}`);
        if (res.data && res.data.status) {
          const data = res.data.data;
          setMeetingPlace(data.meetingplace || "");
          setMeetingDate(data.meetingdate || todayInputDate());
          setOtherEmailsRaw(data.otheremails1 || "");
          setPurpose(data.detail || "");
          // The structure of tasks and representatives would also be populated here
        }
      } catch (err) {
        console.error("Failed to load MOM details:", err);
      } finally {
        setSubmitting(false);
      }
    };
  ...
  const handleSubmitMom = async (mode) => {
    ...
    const payload = {
      ...
      detail: purpose,
      is_draft: mode !== "submit",
      otheremails: selectedEmails.map((e) => e.value),
      otheremails1: otherEmailsRaw,
      representatives: representatives.map((rep) => ({
        userid: rep.representativeuserid === "other" || !rep.representativeuserid ? null : Number(rep.representativeuserid),
        name: rep.representativename,
        organization: rep.representativeorg,
        designation: rep.representativedesig,
        email: rep.representativeemail,
        mobile: rep.representativemobile,
      })),
      tasks: tasks.map((t) => ({
        detail: t.taskdetail,
        timeline: t.tasktimeline,
        responsibilities: t.responsibilities.map((r) => ({
          userid: r.responsibilityuserid === "other" || !r.responsibilityuserid ? null : Number(r.responsibilityuserid),
          name: r.responsibilityname,
          organization: r.responsibilityorg,
          designation: r.responsibilitydesig,
    ...
    try {
      ...
      navigate("/dashboards/master-data/mom-list");
    } catch (err) {
      console.error(err);
      toast.success(
        mode === "submit"
      ...
      <div className="transition-content p-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-gray-150 dark:border-dark-700 pb-4">
          ...
          <Button
            ...
            onClick={() => navigate("/dashboards/master-data/mom-list")}
          >
