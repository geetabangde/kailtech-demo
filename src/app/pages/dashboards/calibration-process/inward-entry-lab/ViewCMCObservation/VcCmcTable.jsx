    ...
    <table className="w-full border-collapse text-[12px] text-gray-700 min-w-max">
      <thead>
        ...
        <tr className="bg-gray-200 text-center text-[12px] font-medium">
          ...
          <th className="border border-gray-300 px-1 py-2 align-bottom">CMC<br />Scope</th>
        </tr>
      </thead>
      <tbody>
        {data.map((row, i) => (
          <tr key={i} className="hover:bg-gray-50 text-center text-[12px]">
            ...
            <td className="border border-gray-300 px-1 py-2">{typeof row.cmc_scope === 'number' ? formatUncertaintyValue(row.cmc_scope, 6) : row.cmc_scope}</td>
          </tr>
        ))}
      </tbody>
    </table>
