...
export function ApproveTrainingPlannerModal({ show, onClose, row, onSuccess }) {
  ...
  useEffect(() => {
    if (show && row?.id) {
      setModules([]);
      setSelectedModuleId("");
      fetchInitialData(row.id);
    }
  }, [show, row]);

  const fetchInitialData = async (employeeId) => {
    try {
      setLoading(true);
      // Fetch available modules
      let avModules = [];
      try {
        const modRes = await axios.get(`/training/get-training-modules`);
        if (modRes.data?.data) avModules = modRes.data.data;
      } catch (e) {
        console.warn("Could not fetch training modules, using fallback data.", e);
        avModules = [
      ...
      // Fetch existing planner for this employee
      try {
        const planRes = await axios.get(`/training/get-training-planner/${employeeId}`);
        if (planRes.data?.data) {
          const planner = planRes.data.data;
          setFormData({ year: planner.year || new Date().getFullYear().toString() });
          setPlannerId(planner.id);
          if (planner.modules) {
            setModules(planner.modules);
          }
        }
      } catch (err) {
        console.warn("Could not fetch training planner details.", err);
      }
    } finally {
      setLoading(false);
    }
  ...
  const addModule = () => {
    ...
    if (modules.some(m => String(m.moduleid || m.id) === String(selectedModuleId))) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(`/training/approve-training-planner`, payload);
      toast.success("Training planner approved successfully");
      if (onSuccess) onSuccess();
      onClose();
    } catch (err) {
      toast.error(err?.response?.data?.message || "Failed to approve planner");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="fixed inset-0 overflow-y-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 p-4 rounded-md dark:bg-dark-800">
            <div>
              ...
              <div className="p-2 bg-gray-200 dark:bg-dark-600 rounded-md text-sm">{formData.year}</div>
            </div>
            <div>
          ...
          <div className="mt-6 flex justify-end gap-3 border-t pt-4 dark:border-dark-600">
            ...
            <Button type="submit" variant="filled" color="secondary" loading={loading} disabled={loading || modules.length === 0}>Approve Planner</Button>
          </div>
        </form>
              </DialogPanel>
            </TransitionChild>
...
ApproveTrainingPlannerModal.propTypes = {
